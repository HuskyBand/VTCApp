...
type StationSummary = {
    ...
    mastery: number; proficient: number; developing: number; novice: number; notStarted: number;
...
const STATUS_COLORS: Record<string, string> = {
    mastery: 'var(--mastery-color)',
    proficient: 'var(--proficient-color)',
    developing: 'var(--notpassed-color)',
    novice: 'var(--notpassed-color)',
    ...
    return (
        <>
            <section id="center">
                <div className="director-page">
                    ...
                    <div className="station-legend">
                        <span>★ mastery</span><span>☆ proficient</span><span>↗ developing</span><span>~ novice</span><span>／ not started</span>
                    </div>
                    <div className="station-legend">
                        Does not count leadership, staff, etc. in grading
                    </div>
                    {overview ? (
                                ...
                                return (
                                    <div key={s.stationId} className="station-stat-card">
                                        ...
                                        <div className="progress-bar-stack">
                                            {(['mastery', 'proficient', 'developing', 'novice', 'not_started'] as const).map(key => (
                                                pcts[key] > 0 && (
                                        ...
                                        <div className="station-stat-counts">
                                            <span className="stat-pill" style={{ background: STATUS_COLORS.mastery, color: 'black' }}>★ {s.mastery}</span>
                                            <span className="stat-pill" style={{ background: STATUS_COLORS.proficient, color: 'white' }}>☆ {s.proficient}</span>
                                            <span className="stat-pill" style={{ background: STATUS_COLORS.developing, color: 'white' }}>↗ {s.developing}</span>
                                            <span className="stat-pill" style={{ background: STATUS_COLORS.novice, color: 'white' }}>~ {s.novice}</span>
                                            <span className="stat-pill" style={{ background: STATUS_COLORS.not_started, color: 'black' }}>／ {s.notStarted}</span>
                                        </div>
                                    </div>
                                );
                            })}
            ...
            <style>{`
                ...
                .director-header { display: flex; align-items: center; margin-bottom: 0.25rem; }
                .director-header h1 { margin: 0 auto; margin-bottom: 0.5rem; text-align: center; }
                .section-heading { margin: 1.75rem 0 0.75rem; font-size: 1.15rem; font-weight: 700; padding-bottom: 0.4rem; }
                .summary-bar { display: flex; gap: 0.75rem; flex-wrap: wrap; justify-content: center; margin-bottom: 0.5rem; }
                .summary-chip { background: #f3f4f6; border-radius: 20px; padding: 0.3rem 0.9rem; font-size: 0.85rem; font-weight: 600; }

                .station-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); max-width: 900px; margin: 0 auto; margin-top: 1rem; gap: 1rem; }
                ...
                .station-stat-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.6rem; }
                ...
                .stat-pill { font-size: 0.8rem; padding: 0.15rem 0.5rem; border-radius: 10px; font-weight: 600; }
                .station-legend { display: flex; gap: 0.6rem; font-size: 0.7rem; color: #9ca3af; flex-wrap: wrap; justify-content: center; }

                .member-select-row { display: flex; gap: 0.75rem; align-items: center; margin-bottom: 1rem; }
                .member-select { flex: 1; max-width: 360px; margin: 0 auto; }
                .member-detail-card { background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 12px; padding: 1.25rem; max-width: 800px; margin: 0 auto; margin-bottom: 1rem; }
                .member-detail-card h3 { margin: 0 0 1rem; }
                .member-instrument { font-weight: 400; color: #6b7280; font-size: 0.9rem; }
                .member-stations-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 0.6rem; }
                ...
                .activity-list { display: flex; flex-direction: column; gap: 0.5rem; max-width: 800px; margin: 0 auto; margin-bottom: 1rem; }
                ...
                .broadcast-panel { padding: 0 1.25rem; max-width: 800px; margin: 0 auto; }
                .broadcast-panel .form-group { margin-bottom: 0.75rem; }

                .notif-list { display: flex; flex-direction: column; gap: 0.75rem; max-width: 800px; }
